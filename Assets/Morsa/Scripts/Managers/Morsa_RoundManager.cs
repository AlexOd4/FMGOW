...
public class Morsa_RoundManager : MonoBehaviour
{
    public static Morsa_RoundManager instance;
    int score = 0;
    bool playing = false;
    ...
    {
        foreach (Morsa_CardBehaviour card in hand) card.untouchable = true;
    ...
    {
        foreach (Morsa_CardBehaviour card in hand) card.untouchable = false;
    ...
    [Header("Slots")]
    [SerializeField] List<Morsa_SlotBehaviour> handSlots;
    [SerializeField] Morsa_SlotBehaviour pastSlot;
    [SerializeField] Morsa_SlotBehaviour presentSlot;
    [SerializeField] Morsa_SlotBehaviour futureSlot;

    [Header("Cards")]
    [SerializeField] public List<Morsa_Card> deck = new();
    [SerializeField] public GameObject cardPrefab;
    List<Morsa_CardBehaviour> hand = new();
    public Morsa_CardBehaviour selectedCard;
    public Morsa_CardBehaviour pastCard;
    public Morsa_CardBehaviour presentCard;
    public Morsa_CardBehaviour futureCard;
    ...
    public void AbortRound()
    {
        if (playing)
        {
            ...
            foreach (Morsa_CardBehaviour card in hand)
            {
                deck.Add(card.card);
                card.Discard();
            }
        ...
        if (Input.touchCount >= 1)
        {
            switch (Input.GetTouch(0).phase)
            {
                case TouchPhase.Began:
                    ...
                    if (hit && hit.GetComponent<Morsa_CardBehaviour>() && !hit.GetComponent<Morsa_CardBehaviour>().untouchable) Select(hit.GetComponent<Morsa_CardBehaviour>());
                    break;
    ...
    public void NextRound()
    {
        if (playing)
        {
            ...
            while (hand.Count < 6)
            {
                ...
                card.GetComponent<Morsa_CardBehaviour>().card = deck[draw];
                card.transform.position = handSlots[hand.Count].transform.position;

                // Add drawn card to HAND
                hand.Add(card.GetComponent<Morsa_CardBehaviour>());
    ...
    public void WalkIn()
    {
        if (playing)
        {
            currentClient.GetComponent<Morsa_ClientBehaviour>().SetProperties(pastSlot, presentSlot, futureSlot);
    ...
    public void Select(Morsa_CardBehaviour card)
    {
        card.Select();
        selectedCard = card;
    ...
    public void Deselect()
    {
        ...
        foreach (Morsa_CardBehaviour card in hand) card.Deselect();
